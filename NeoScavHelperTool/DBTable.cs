...
namespace NeoScavModHelperTool
{
    ...
    public enum EDBTable
    {
        ...
        [
            ...
            IdNameColumnsPositions(new int[] { 0, 2 })
            ]
        eBattleMoves,
        ...
        [
            ...
            IdNameColumnsPositions(new int[] { 0, 3 })
            ]
        eForbiddenHexes,
        ...
        [
            ...
            IdNameColumnsPositions(new int[] { -1, 1 })
            ]
        eGameVars,
        ...
        [
            ...
            IdNameColumnsPositions(new int[] { -1, 0 })
            ]
        eImages,
    ...
    namespace DBTableAttributes
    {
        ...
        public class IdNameColumnsPositionsAttribute : Attribute
        {
            private int[] _value;
            public IdNameColumnsPositionsAttribute(int[] value)
            {
                _value = value;
            }
            public int[] Value
        ...
        public class DBTableAttributtesFetcher
        {
            ...
            private static Hashtable _IdNameColumnsPositions = new Hashtable();
            ...
            public static string GetNameSufix(string table_name_sufix)
            {
                string output = null;
            ...
            public static int GetIdColumnPosition(EDBTable value)
            {
                int output = -1;
                Type type = typeof(EDBTable);

                if (_IdNameColumnsPositions.ContainsKey(value))
                    output = (_IdNameColumnsPositions[value] as IdNameColumnsPositionsAttribute).Value[0];
                else
                {
                    //Look for our 'IdNameColumnsPositionsAttribute' in the field's custom attributes
                    FieldInfo fi = type.GetField(value.ToString());
                    IdNameColumnsPositionsAttribute[] attrs = fi.GetCustomAttributes(typeof(IdNameColumnsPositionsAttribute), false) as IdNameColumnsPositionsAttribute[];
                    if (attrs.Length > 0)
                    {
                        _IdNameColumnsPositions.Add(value, attrs[0]);
                        output = attrs[0].Value[0];
                    }
                }
                return output;
            }

            public static int GetNameColumnPosition(EDBTable value)
            {
                int output = -1;
                Type type = typeof(EDBTable);

                if (_IdNameColumnsPositions.ContainsKey(value))
                    output = (_IdNameColumnsPositions[value] as IdNameColumnsPositionsAttribute).Value[1];
                else
                {
                    //Look for our 'IdNameColumnsPositionsAttribute' in the field's custom attributes
                    FieldInfo fi = type.GetField(value.ToString());
